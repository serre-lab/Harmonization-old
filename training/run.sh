...
#
echo "\n\n"
echo "\t Trying to start tpu vms: ($1) zone: ($2) tpu: ($3) (learning_rate=$4, lambda=$5)"
echo "\n\n"

if [ "$3" = "v4-8" ]; then
    gcloud alpha compute tpus tpu-vm create $1 --zone=$2 --accelerator-type=$3 --version=v2-alpha-tpuv4
else
    gcloud alpha compute tpus tpu-vm create $1 --zone=$2 --accelerator-type=$3 --version=tpu-vm-tf-2.7.0
fi
...
# sending files
gcloud alpha compute tpus tpu-vm scp --recurse '../meta-predictor-tpu/'* $1:~/ --zone=$2

sleep 3

# install and running the py script
install="./scripts/install.sh"
pyrun="screen -dmS training bash -c 'python3 run.py $4 $5'"
cmd="$install && $pyrun"
